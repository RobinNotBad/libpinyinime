...
int getch()
{
	...
	ch = read(STDIN_FILENO, &c, 1) < 0 ? -1 : (unsigned char)c;
	...
	while (1)
	{
		...
		if (try_jianpin)
		{
			...
			for (auto i = words_to_choose.begin(); i != words_to_choose.end(); i++)
				cout << i - words_to_choose.begin() + 1 << ".", cout << wstring2string(*i) << "  ";
			...
			cout << "输入结果：";
			cout << wstring2string(words_to_choose[word_choose - 1]) << endl;
		...
		cout << "输入结果：";
		cout << wstring2string(final_word) << endl;
